...
public class SortingAlgorithmEvaluationFramework {
	...
	public static void main(String[] args) {
		JOptionPane.showMessageDialog(null, "Selezionare una cartella di destinazione per i file di output");
		...
		try {
			o = new PrintStream(new File(fc.getSelectedFile().getAbsolutePath() + "/" + "evalfram.csv"));
			sequences = new PrintStream(new File(fc.getSelectedFile().getAbsolutePath() + "/" + "sequences.csv"));
		...
		for (n = SortingAlgorithmEvaluationFrameworkParameters.MIN_LENGTH; n <= SortingAlgorithmEvaluationFrameworkParameters.MAX_LENGTH; n += SortingAlgorithmEvaluationFrameworkParameters.INCREMENTO_LUNGHEZZA) {
			...
			for (int i = 0; i < SortingAlgorithmEvaluationFrameworkParameters.NUMBER_OF_SAMPLES_PER_LENGTH; i++) {
				...
				// Genero la sequenza
				for (int j = 0; j < n; j++) {
					Integer x = new Integer(randomGenerator
							.nextInt(SortingAlgorithmEvaluationFrameworkParameters.MAX_GENERATED_INTEGER));
